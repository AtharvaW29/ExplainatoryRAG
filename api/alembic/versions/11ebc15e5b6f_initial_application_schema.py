...
Revision ID: 11ebc15e5b6f
Revises:
Create Date: 2026-07-25 14:39:32.219708
...
def upgrade() -> None:
    ...
    op.create_index('idx_mastery_concept', 'concept_mastery', ['concept_id'], unique=False)
    op.create_index('idx_mastery_user', 'concept_mastery', ['user_id'], unique=False)
    ...
    sa.Column('preferred_explanation_style', sa.String(length=100), nullable=True),
    ...
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('NOW()'), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['exp_session_id'], ['explanation_sessions.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table('explaination_embeddings',
    sa.Column('session_id', sa.UUID(), nullable=False),
    sa.Column('embedding', pgvector.sqlalchemy.vector.VECTOR(dim=1536), nullable=False),
    sa.ForeignKeyConstraint(['session_id'], ['explanations.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('session_id')
    )
    ...
    op.create_table('retrieval_metatdata',
    sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
    sa.Column('explanation_id', sa.UUID(), nullable=False),
